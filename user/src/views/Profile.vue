  ...
  setup() {
    ...
    const fetchUserInfo = async () => {
      ...
      try {
        ...
        if (!userId || !token) {
          alert('用户ID丢失，可能需要重新登录。3秒后跳转到登录页面...')
          setTimeout(() => {
        ...
        try {
          ...
          try {
            fallbackUserInfo = userInfoData ? JSON.parse(userInfoData) : null
          } catch (e) {
            fallbackUserInfo = null
          }
          
          const username = fallbackUserInfo?.userName || localStorage.getItem('userName') || `用户${userId.slice(-4)}`
