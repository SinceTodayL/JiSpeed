...
function parseAndStoreAuthFromURL() {
  ...
  if (userId && token) {
    // 存储认证信息到 AuthStorage
    setAuthStorage(token, userId, 2); // userType 设为 2 表示商家
...
async function setupApp() {
  ...
  if (hasAuthFromURL) {
    // 延迟执行以确保store已经初始化
    setTimeout(() => {
      const { useMerchantStore } = require('./store/modules/merchant');
      const merchantStore = useMerchantStore();
      merchantStore.triggerAuthUpdate();
      console.log('已触发merchantStore更新，当前merchantId:', merchantStore.merchantId);
    }, 100);
  }
